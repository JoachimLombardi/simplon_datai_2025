...
from titanic import stat_survive
        ...
        if sexe == "female":
            ...
            for i, v in enumerate(list(stat_survive("Age", "male").values())):
             ...
             if categorie == "Age":
                ...
                sns.barplot(x=list(stat_survive("Sexe").keys()), y=list(stat_survive("Sexe").values()))
                ax.set(title="Taux de survie des personnes par sexe",
                       xlabel="sexe",
                       ylabel="taux de survie")
                for i, v in enumerate(list(stat_survive("Sexe").values())):
                    ax.annotate(f"{v * 100:.2f}%", (i, v), ha='center', va='bottom', fontsize=10)
             elif categorie == "Pclasse":
                sns.barplot(x=list(stat_survive("Pclasse").keys()), y=list(stat_survive("Pclasse").values()))
                ax.set(title="Taux de survie des personnes par classe",
                xlabel="classe",
                ylabel="taux de survie")
                for i, v in enumerate(list(stat_survive("Pclasse").values())):
                     ax.annotate(f"{v * 100:.2f}%", (i, v), ha='center', va='bottom', fontsize=10)
